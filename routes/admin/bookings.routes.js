  ...
  try {
    ...
    try {
      ...
      await sendEmail({
        ...
        html: `
          <h2>🎵 Class Slot Confirmed</h2>
          <p>Hi ${booking.userName || 'Student'},</p>
          <p>Your slot request has been approved! Here are your confirmed class details:</p>
          <h3>Slot Details:</h3>
          <ul>
            <li><strong>Class:</strong> ${classItem}</li>
            <li><strong>Date:</strong> ${slotDateLabel}</li>
            <li><strong>Time:</strong> ${formatTime12(startTime)} – ${formatTime12(endTime)}</li>
          </ul>
          ${calendarInvite ? '<p>A calendar invite is attached — add it to your calendar to get a reminder.</p>' : ''}
      ...
      for (const adminEmail of adminNotificationEmails) {
        ...
        try {
          await sendEmail({
            ...
            html: `
              <h2>Class Slot Approved</h2>
              <p>Slot request approved by ${req.user.name}.</p>
              <ul>
                <li><strong>Student:</strong> ${booking.userName || 'N/A'} (${booking.userEmail || 'N/A'})</li>
                <li><strong>Class:</strong> ${classItem}</li>
                <li><strong>Date:</strong> ${slotDateLabel}</li>
                <li><strong>Time:</strong> ${formatTime12(startTime)} – ${formatTime12(endTime)}</li>
              </ul>
      ...
      try {
        await sendEmail({
          ...
          html: `
            <h2>🎵 Class Slot Confirmed</h2>
            <p>Hi ${booking.userName || 'Student'},</p>
            <p>Your class slot has been booked by the studio team.</p>
            <h3>Slot Details:</h3>
            <ul>
              <li><strong>Class:</strong> ${classItem}</li>
              <li><strong>Date:</strong> ${slotDateLabel}</li>
      ...
      for (const adminEmail of adminNotificationEmails) {
        ...
        try {
          await sendEmail({
            ...
            html: `
              <h2>Class Slot Booked by Admin</h2>
              <p>Slot assigned by ${req.user.name}.</p>
              <ul>
                <li><strong>Student:</strong> ${booking.userName || 'N/A'} (${booking.userEmail || 'N/A'})</li>
                <li><strong>Class:</strong> ${classItem}</li>
                <li><strong>Date:</strong> ${slotDateLabel}</li>
...
<style>
  ...
  @media only screen and (max-width:600px){
    .eq{padding:6px!important}
    .hdr{padding:14px!important}
    .hdr h2{font-size:17px!important}
    .body{padding:12px!important}
    .hdr-table{display:block!important}
    .hdr-left,.hdr-right{display:block!important;width:100%!important;padding:0!important}
    .order-box{margin-top:10px!important}
    .status-card-table{display:block!important}
    .status-card-left,.status-card-right{display:block!important;width:100%!important;padding:0!important;margin-bottom:10px!important}
    .sc{margin-bottom:10px!important}
    .detail-table td{font-size:12px!important}
    .totals-table td{font-size:12px!important}
  }
  @media (prefers-color-scheme:dark){
    body,.eq,.card{background-color:#eef2f7!important;color:#1f2937!important}
    .body,.sc,.booking-card,.totals-card,.payment-card,.cta,.attach.ok,.attach.warn{background-color:#ffffff!important;color:#1f2937!important}
...
<div class="eq">
  <div class="card">
    ...
    <div class="body">
      ...
      <!-- Status cards — stacks to single column on mobile -->
